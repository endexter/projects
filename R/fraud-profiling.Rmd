...
The above graph shows the discrepancy between when an account commits the fraudulent activity versus when the account is actually closed and charged-off, especially noticed by the sporadic spikes in termination date versus negative account date. Previously, much of the account maintenance and fraud mitigation was performed by Radius.
...
df_states[order(-rate)][1:10]

# Customer age
df[, .(mean_customer_age = round(mean(customer_age),2),
       median_customer_age = median(customer_age)), by = .(is_charged_off)]

# days to first summit debit card use
df[, .(mean_days_till_first_debit = round(mean(days_to_first_summit_debit_card_use, na.rm= TRUE),2), 
	median_days_till_first_debit = median(days_to_first_summit_debit_card_use, na.rm=TRUE)), by = .(is_charged_off)]

# Inital funding
df[, .(mean_initial_funding_amount = round(mean(first_account_funding_amount, na.rm= TRUE),2), 

...
# Top states
#df_states <- df[, .N, by = .(state, is_charged_off)]
#df_states[, total := sum(N), by = is_charged_off][, rate := round(N/total, 4), by = is_charged_off]

#print(xtable(df_states[order(-rate)][1:10], floating=TRUE, caption = "High Frequency Customer States"), #include.rownames=TRUE, scalebox='0.90', caption.placement = "top")

# Customer age
#print(xtable(df[, .(mean_customer_age = round(mean(customer_age),2),
#       median_customer_age = median(customer_age)), by = .(is_charged_off)], floating=TRUE, caption = "Customer Age #(Years)"), include.rownames=TRUE, scalebox='0.90', caption.placement = "top")

# days to first summit debit card use
df_debit <- df[, .(mean_days_till_first_debit = round(mean(days_to_first_summit_debit_card_use, na.rm= TRUE),2), 
	median_days_till_first_debit = median(days_to_first_summit_debit_card_use, na.rm=TRUE)), by = .(is_charged_off)]

print(xtable(df_debit, floating=TRUE, caption = "Days to First Debit Card Usage"), include.rownames=TRUE, scalebox='0.90', caption.placement = "top")

# Inital funding
df_fund <- df[, .(mean_initial_funding_amount = round(mean(first_account_funding_amount, na.rm= TRUE),2), 
	median_initial_funding_amount = median(first_account_funding_amount, na.rm=TRUE)), by = .(is_charged_off)]

print(xtable(df_fund, floating=TRUE, caption = "Initial Funding"), include.rownames=TRUE, scalebox='0.90', caption.placement = "top")
...
## Users who went negative after 200 + days

Comparing deposite counts and withdrawals among users who first went negative after 200 days, those who stayed negative, and those who did not.
...
df2 <- df[, .(#count = .N,
	avg_deposits = mean(total_deposits, na.rm=TRUE),
	median_deposits = median(total_deposits, na.rm=TRUE),
	avg_withdrawals = mean(total_withdrawals, na.rm=TRUE),
	median_withdrawals = median(total_withdrawals, na.rm=TRUE)), 
	by = .(went_and_stayed_neg, neg_after_200_days)][order(went_and_stayed_neg, neg_after_200_days)]

print(xtable(df2, floating=TRUE, caption = "Account activity during first 200 days"), include.rownames=FALSE, scalebox='0.90', caption.placement = "top")
...
df2 <- df[, .N, by = .(is_charged_off, browser_name)][, head(.SD, 5), by = is_charged_off][
  order(is_charged_off, -N)]

print(xtable(df2, floating=TRUE, caption = "Top 5 browsers between fraud and non-fraud"), include.rownames=FALSE, scalebox='0.90', caption.placement = "top")
...
print(xtable(df2, floating=TRUE, caption = "Top 5 states between fraud and non-fraud"), include.rownames=FALSE, scalebox='0.90', caption.placement = "top")
...
df2 <- df[, .N, by = .(is_charged_off, email_domain)][, head(.SD, 5), by = is_charged_off][
  order(is_charged_off, -N)]

print(xtable(df2, floating=TRUE, caption = "Top 5 email domains between fraud and non-fraud"), include.rownames=FALSE, scalebox='0.90', caption.placement = "top")
...
- **Why is ATM Severity higher the highest severity**

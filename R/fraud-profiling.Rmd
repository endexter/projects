...
- **Fraud Rate:** The proportion of fraudulent accounts versus non-fradulent accounts, calculated by any of the above date definitions.
...
```{r, echo = FALSE}
#df[, ever_terminated := ifelse(!is.na(termination_date), TRUE, FALSE)]

df2 <- df[is_charged_off == TRUE]

df3 <- df[, .(hard_loss = sum(hard_loss_charge_off), lost_fees = sum(fees_charged_off)), 
	by = .(negative_month, negative_week)]
	
df4 <- df[neg_balance_ever == TRUE, .N, by = .(negative_month, negative_week, is_charged_off)]
#df4 <- df[neg_balance_ever == TRUE, .N, by = .(negative_month, negative_week, ever_terminated)]

#df[, .N, by = .(neg_balance_ever, is_charged_off)]
